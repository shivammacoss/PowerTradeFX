...
// If user is under an admin, return admin-specific bank settings
  ...
  try {
    ...
    if (userId) {
      ...
      if (user && user.assignedAdmin) {
        // Get admin-specific bank settings
        const adminSettings = await AdminSettings.findOne({ adminId: user.assignedAdmin })
        if (adminSettings && adminSettings.isConfigured?.bankSettings && adminSettings.bankSettings) {
          // Convert admin bank settings to payment method format
          const bankSettings = adminSettings.bankSettings
          const paymentMethods = []
          
          // Add bank transfer if configured
          ...
          // Add crypto addresses if configured
          if (bankSettings.cryptoAddresses && bankSettings.cryptoAddresses.length > 0) {
            bankSettings.cryptoAddresses.forEach((crypto, index) => {
              paymentMethods.push({
                _id: `admin-crypto-${index}`,
                type: 'crypto',
                cryptoCurrency: crypto.currency,
                cryptoNetwork: crypto.network,
                cryptoAddress: crypto.address,
                qrCodeImage: crypto.qrCode,
                isActive: true
              })
            })
          }
    ...
    // Default: return global payment methods
