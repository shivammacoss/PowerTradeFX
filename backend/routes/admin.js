...
router.put('/users/:id/block', async (req, res) => {
  try {
    ...
    if (blocked && emailSettings.accountStatusNotificationsEnabled) {
      await sendEmail('account_blocked', user.email, {
        name: user.firstName,
...
router.put('/users/:id/ban', async (req, res) => {
  try {
    ...
    // Send email notification if enabled
    const emailSettings = await EmailSettings.getSettings()
    if (banned && emailSettings.accountStatusNotificationsEnabled) {
      await sendEmail('account_banned', user.email, {
        name: user.firstName,
