...
router.post('/deposit', async (req, res) => {
  try {
    ...
    // Require reference and screenshot for deposit
  ...
  try {
    ...
    if (!kycRecord) {
      return res.status(400).json({ 
        ...
        kycRequired: true,
        error: 'Please complete your KYC verification before making a withdrawal request'
      })
    }
...
router.get('/admin/transactions', verifyAdminToken, async (req, res) => {
  try {
    const { adminId: filterAdminId } = req.query
    ...
    // Super admin and employee without filter see ALL transactions
...
router.put('/admin/approve/:id', async (req, res) => {
  try {
    ...
    if (transaction.type === 'Deposit') {
      ...
      // Deduct from admin wallet if user is under an admin
      if (assignedAdminId) {
        const adminWallet = await AdminWallet.findOne({ adminId: assignedAdminId })
        if (adminWallet) {
          adminWallet.balance -= transaction.amount
          adminWallet.totalGivenToUsers += transaction.amount
          ...
          await AdminWalletTransaction.create({
            ...
            description: `Deposit approved for user ${user.email}`,
      ...
      // Add to admin wallet if user is under an admin (withdrawal returns money to admin)
      if (assignedAdminId) {
        const adminWallet = await AdminWallet.findOne({ adminId: assignedAdminId })
        if (adminWallet) {
          adminWallet.balance += transaction.amount
          ...
          await AdminWalletTransaction.create({
            ...
            amount: transaction.amount,
            description: `Withdrawal approved for user ${user.email}`,
            relatedUserId: transaction.userId,
            relatedTransactionId: transaction._id
          })
        }
