...
router.post('/deposit', async (req, res) => {
  try {
    ...
    // Send deposit request email if enabled
    const emailSettings = await EmailSettings.getSettings()
    if (emailSettings.depositNotificationsEnabled) {
      const user = await User.findById(userId)
      if (user) {
        await sendEmail('deposit_request', user.email, {
          name: user.firstName,
...
router.post('/withdraw', async (req, res) => {
  try {
    ...
    // Send withdrawal request email if enabled
    const emailSettings = await EmailSettings.getSettings()
    if (emailSettings.withdrawalNotificationsEnabled) {
      const user = await User.findById(userId)
      if (user) {
        await sendEmail('withdrawal_request', user.email, {
          name: user.firstName,
          amount: `$${amount.toFixed(2)}`,
          paymentMethod: paymentMethod || 'N/A',
          date: new Date().toLocaleString()
...
router.put('/transaction/:id/approve', async (req, res) => {
  try {
    ...
    const shouldSend = isDeposit ? emailSettings.depositNotificationsEnabled : emailSettings.withdrawalNotificationsEnabled
    if (shouldSend) {
      const user = await User.findById(transaction.userId)
      if (user) {
        const templateName = isDeposit ? 'deposit_approved' : 'withdrawal_approved'
        await sendEmail(templateName, user.email, {
          name: user.firstName,
...
router.put('/transaction/:id/reject', async (req, res) => {
  try {
    ...
    if (shouldSend) {
      const user = await User.findById(transaction.userId)
      if (user) {
        const templateName = isDeposit ? 'deposit_rejected' : 'withdrawal_rejected'
        await sendEmail(templateName, user.email, {
          name: user.firstName,
          amount: `$${transaction.amount.toFixed(2)}`,
          reason: adminRemarks || 'No reason provided'
        })
      }
    }
