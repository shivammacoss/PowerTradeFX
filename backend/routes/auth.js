...
router.post('/signup', async (req, res) => {
  try {
    ...
    // Find admin by slug if provided, otherwise assign to super admin
    ...
    // If no admin assigned via slug, assign to super admin
    if (!assignedAdmin) {
      const superAdmin = await Admin.findOne({ role: 'SUPER_ADMIN', status: 'ACTIVE' })
      if (superAdmin) {
        assignedAdmin = superAdmin._id
        console.log(`[Signup] User ${email} auto-assigned to Super Admin`)
      }
    }
    ...
    if (smtpEnabled) {
      ...
      try {
        await sendTemplateEmail('password_reset', email, {
          ...
          supportEmail: emailSettings?.fromEmail || 'support@PowerTradeFX.com',
          year: new Date().getFullYear().toString()
        })
