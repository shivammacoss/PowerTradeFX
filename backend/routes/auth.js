...
// GET /api/auth/signup-settings - Check if OTP is required for signup
router.get('/signup-settings', async (req, res) => {
  try {
    const settings = await EmailSettings.getSettings()
    res.json({ 
      otpRequired: settings.signupOtpEnabled,
      welcomeEmailEnabled: settings.welcomeEmailEnabled
    })
  } catch (error) {
    res.json({ otpRequired: false, welcomeEmailEnabled: true })
  }
})

// POST /api/auth/send-signup-otp - Send OTP for signup verification
router.post('/send-signup-otp', async (req, res) => {
  try {
    ...
    if (!settings.signupOtpEnabled) {
      return res.status(400).json({ message: 'OTP verification is not enabled', otpDisabled: true })
    }
    ...
    // Send OTP email
    const emailResult = await sendEmail('signup_otp', email, {
      name: firstName || 'User',
      otp
    })
...
router.post('/verify-signup-otp', async (req, res) => {
  try {
    const { firstName, email, phone, countryCode, password, adminSlug, otp } = req.body
    ...
    // Find OTP
    const otpRecord = await OTP.findOne({ email, type: 'signup', isUsed: false })
    ...
    if (emailSettings.welcomeEmailEnabled) {
...
// POST /api/auth/signup - Direct signup (without OTP - for backward compatibility)
